...
OUTPUT_DIR = '/ibdc-scratch2/home/IBDCHPCU0095/qubis-hiq/experiments/paper1/results/exp1a'
    ...
    try:
        ...
        if USE_GPU:
            ...
            try:
                ...
                str_counts = {(format(k, f'0{n_q}b') if isinstance(k, int) else str(k)): v
...
def run_full():
    ...
    if n_arg:
        np.random.seed(42)
        idx = np.random.choice(len(all_seqs), n_arg, replace=False)
        seqs = [all_seqs[i] for i in sorted(idx)]
    else:
        seqs = all_seqs

    n_workers = cpu_count()
    print(f"[{datetime.now()}] Exp 1A: {len(seqs)} seqs | {n_workers} CPUs | GPU={USE_GPU}", flush=True)
    ...
    # Pad to uniform feature vector length
