...
for i, seq in enumerate(seqs):
      qc_aer = build_circuit(seq, stem_pairs=[], trainable_params=params, include_measurement=True)
...
results = {
      'experiment': '1D_12mer',
      'seq_len': SEQ_LEN, 'n_qubits': 2 * SEQ_LEN,
      'n_seqs': N_SEQS, 'n_shots': N_SHOTS,
      'backend': 'ibm_fez', 'circuit_version': 'v2_ry_cx_ry',
      'sequences': seqs, 'seq_labels': seq_labels, 'gc_contents': gc_vals,
      'baseline': {
                'job_id': job_id_base, 'exec_seconds': time_base,
                'mean_cosine': mean_base, 'std_cosine': std_base,
                'min_cosine': float(np.min(cos_base)), 'max_cosine': float(np.max(cos_base)),
                'success_95': bool(mean_base > 0.95), 'by_group': group_stats(cos_base),
                'cosines': cos_base,
      },
      'error_mitigated': {
                'job_id': job_id_em, 'exec_seconds': time_em,
                'mean_cosine': mean_em, 'std_cosine': std_em,
                'min_cosine': float(np.min(cos_em)), 'max_cosine': float(np.max(cos_em)),
                'success_97': bool(mean_em > 0.97),
                'dd_sequence': 'XY4', 'twirling': 'gate+measure',
                'by_group': group_stats(cos_em), 'cosines': cos_em,
      },
      'improvement': {
