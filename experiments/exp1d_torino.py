...
cos_torino      = [float(sk_cosine([sim_features[i]], [hw_features[i]])[0][0]) for i in range(N_SEQS)]
...
output = {
      ...
      'sequences': seqs, 'seq_labels': seq_labels,
      'ibm_torino': {
                'mean_cosine': mean_cos_torino, 'std_cosine': std_cos_torino,
                'min_cosine': float(np.min(cos_torino)), 'max_cosine': float(np.max(cos_torino)),
      ...
      'cross_platform': {
                'ibm_fez_mean': IBM_FEZ_MEAN, 'ibm_fez_std': IBM_FEZ_STD,
                'ibm_torino_mean': mean_cos_torino, 'ibm_torino_std': std_cos_torino,
                'delta': delta, 'portable': bool(abs(delta) < 0.01),
      },
}
